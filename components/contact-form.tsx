...
export function ContactForm() {
	...
	async function sendToTelegram(payload: {
		...
		try {
			...
			if (payload.tell) body.append('tell', payload.tell)
			if (payload.thema) body.append('thema', payload.thema)
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		const whatsappMessage = `Новое сообщение от клиента:\n\nИмя: ${
			...
			selectedBranch?.label || 'Не выбран'
		...
		const whatsappNumber = selectedBranch?.whatsapp || '+77712222267'

		// If on mobile - open WhatsApp. On desktop - send via our PHP -> Telegram endpoint
		if (isMobile) {
			window.open(
			...
			await sendToTelegram({
				name: formData.name || '',
				tell: formData.phone || '',
				thema: `Заявка: ${selectedBranch?.label || 'Не выбран'}`,
			})
		}
